...
export const TomorrowPlanView: React.FC = () => {
  const [activeSubTab, setActiveSubTab] = useState<'proposal' | 'comparison' | 'history'>('proposal');
  ...
  const handleDecisionChange = (idx: number, newDecision: string) => {
  ...
  return (
    <div className="view-content" style={{ maxWidth: '1350px', margin: '0 auto' }}>
      {/* Top Title & Sub-tabs */}
      <div style={{ 
        background: '#ffffff', 
        ...
        <div style={{ display: 'flex', background: '#f1f5f9', padding: '0.25rem', borderRadius: '10px', gap: '0.25rem' }}>
          ...
          <button 
            onClick={() => { setActiveSubTab('comparison'); fetchComparison(); }}
            style={{ 
                    ...
                    <td>
                      {isApproved ? (
                        ...
                        <select 
                          ...
                          value={item.plannedDecision}
                          onChange={(e) => handleDecisionChange(idx, e.target.value)}
                          style={{ height: '34px', fontSize: '0.82rem', fontWeight: 700, padding: '0.2rem 0.5rem' }}
                        >
        ...
        <div>
          ...
          <div className="table-container">
            <table className="custom-table">
              ...
              <tbody>
                {(comparisonData?.comparison || planItems).map((comp: any, idx: number) => (
                  <tr key={idx}>
