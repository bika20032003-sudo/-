...
interface RoadProgressViewProps {
  onNavigateTab?: (tab: string) => void;
}

export const RoadProgressView: React.FC<RoadProgressViewProps> = ({ onNavigateTab }) => {
  ...
  const fetchRoadProgress = async () => {
    try {
      setIsLoading(true);
      const res = await fetch(`http://localhost:5000/api/road-progress?sector=${selectedSector}`);
  ...
  const handleSaveEdit = async (id: number) => {
    try {
      const res = await fetch('http://localhost:5000/api/road-progress/update-daily', {
        method: 'POST',
  ...
  const handleExportExcel = () => {
    ...
    const exportData = projectData.items.map((item: any, idx: number) => ({
      'م': idx + 1,
      'القطاع': item.sector,
  ...
  const sectorAItems = (projectData?.items || []).filter((i: any) => i.sector.includes('A'));
  const sectorBItems = (projectData?.items || []).filter((i: any) => i.sector.includes('B'));
  ...
  return (
    <div className="view-content" style={{ maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {(selectedSector === 'all' || selectedSector === 'A') && (
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    {editingId === item.id ? (
                      <input 
                        ...
                        value={editMeters} 
                        onChange={(e) => setEditMeters(parseFloat(e.target.value) || 0)}
                        style={{ width: '90px', height: '32px', padding: '0.2rem' }}
                      />
                    ) : (
                  ...
                  <td>
                    {editingId === item.id ? (
                      ...
                      <button 
                        onClick={() => { setEditingId(item.id); setEditMeters(item.todayMeters); setEditNotes(item.notes || ''); }}
                        style={{ background: '#f1f5f9', border: 'none', borderRadius: '4px', padding: '0.25rem 0.5rem', cursor: 'pointer', color: '#2563eb' }}
                        title="تحديث منجز اليوم"
                      >
                        <Edit3 size={14} />
                      </button>
                    )}
      ...
      {(selectedSector === 'all' || selectedSector === 'B') && (
        <div className="dashboard-white-card" style={{ padding: '1.5rem', marginBottom: '1.75rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
