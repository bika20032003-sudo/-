...
import { FileText, CheckCircle2, X, Camera, FileSpreadsheet, AlertTriangle, Eye, Calendar, Layers, Sparkles, Check } from 'lucide-react';
import * as XLSX from 'xlsx';
export const UploadReportsView = ({ onNavigateTab, currentUser }) => {
    // File & Form State
    ...
    const defaultSector = (currentUser?.sector && currentUser.sector !== 'all') ? currentUser.sector : 'القطعة A';
    ...
    const processAndExtractData = (file, fileContent) => {
    ...
    const handleConfirmAndCommit = async () => {
        ...
        try {
            const userRole = currentUser?.role || 'مشرف القطاع';
            const isSupervisor = userRole.includes('مشرف');
            const reportStatus = isSupervisor ? 'pending_review' : 'approved';
            const uploaderName = currentUser?.name || (sector.includes('A') ? 'مشرف القطاع (A)' : 'مشرف القطاع (B)');
            ...
            const response = await fetch('http://localhost:5000/api/reports/upload', {
                ...
                body: JSON.stringify({
                    ...
                    salesAmount: extractedData.sharshoorAmount || '750',
    ...
    return (<div className="view-content" style={{ maxWidth: '900px', margin: '0 auto' }}>
      {/* Top Header */}
      <div style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {onNavigateTab && (<div style={{ display: 'flex', gap: '0.65rem' }}>
      ...
      <div className="dashboard-white-card" style={{ padding: '2.25rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          <div className="form-group">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <label className="form-label" style={{ fontSize: '0.85rem' }}>القطاع الميداني</label>
              {currentUser?.sector && currentUser.sector !== 'all' && (
                <span style={{ fontSize: '0.7rem', color: '#2563eb', fontWeight: 800, background: '#eff6ff', padding: '0.1rem 0.4rem', borderRadius: '4px' }}>
                  مقفل بحسب صلاحية الحساب
                </span>
              )}
            </div>
            <select 
              ...
              disabled={Boolean(currentUser?.sector && currentUser.sector !== 'all')}
              style={currentUser?.sector && currentUser.sector !== 'all' ? { background: '#f8fafc', cursor: 'not-allowed', fontWeight: 700 } : {}}
            >
              <option value="القطعة A">القطاع (A)</option>
              <option value="القطعة B">القطاع (B)</option>
              {(!currentUser?.sector || currentUser.sector === 'all') && (
        ...
        {/* Hidden File Input */}
        <input type="file" ref={fileInputRef} onChange={handleFileSelect} accept="image/*,.pdf,.xlsx,.xls,.csv" style={{ display: 'none' }}/>

        {/* Upload Box / Dropzone */}
        {attachedFile || fileName ? (<div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              {attachedFile && attachedFile.startsWith('data:image') ? (<img src={attachedFile} alt="معاينة" style={{ width: '80px', height: '80px', objectFit: 'cover', borderRadius: '12px', border: '2px solid #fff', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}/>) : (<div style={{ width: '70px', height: '70px', borderRadius: '12px', background: '#dcfce7', color: '#16a34a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button type="button" className="secondary-action-btn" style={{ background: '#fff' }} onClick={() => setShowReviewModal(true)}>
                <Eye size={15}/>
                <span>مراجعة البيانات المستخرجة</span>
              </button>
              <button type="button" style={{ background: '#fee2e2', color: '#dc2626', border: 'none', padding: '0.5rem 0.85rem', borderRadius: '8px', cursor: 'pointer', fontWeight: 700, fontSize: '0.82rem' }} onClick={() => { setAttachedFile(null); setFileName(''); setFileSize(''); }}>
                حذف
              </button>
            </div>
          </div>) : (<div style={{
        ...
        <div className="form-group" style={{ marginBottom: '1.5rem' }}>
          ...
          <input type="text" className="form-input" placeholder="مثال: تم إيقاف الحفار للصيانة، زيادة إنتاج الكسارة الشمالية..." value={notes} onChange={(e) => setNotes(e.target.value)}/>
        </div>

        {/* Progress bar */}
        {isUploading && (<div style={{ marginBottom: '1.25rem' }}>
        ...
        {/* Submit Button */}
        <button type="button" className="primary-action-btn" disabled={!attachedFile && !fileName} onClick={() => setShowReviewModal(true)} style={{
            width: '100%',
            height: '52px',
      ...
      {showReviewModal && (<div className="modal-backdrop">
          <div className="modal-card" style={{ maxWidth: '650px', borderRadius: '16px' }}>
            ...
            <div style={{ padding: '1.25rem 0' }}>
              ...
              {extractedData.detectedIssues.length > 0 && (<div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '10px', padding: '0.85rem 1rem', marginBottom: '1.25rem' }}>
                  ...
                  <ul style={{ paddingRight: '1.2rem', margin: 0, fontSize: '0.82rem', color: '#92400e' }}>
                    {extractedData.detectedIssues.map((iss, idx) => (<li key={idx} style={{ marginBottom: '0.2rem' }}>{iss}</li>))}
                  </ul>
                </div>)}
            ...
            <div className="modal-actions" style={{ display: 'flex', gap: '0.75rem' }}>
              <button type="button" className="primary-action-btn" onClick={handleConfirmAndCommit} style={{ flex: 1.5, height: '46px', justifyContent: 'center' }}>
                <CheckCircle2 size={18}/>
                <span>اعتماد التقرير وترحيل البيانات</span>
              </button>

              <button type="button" className="secondary-action-btn" onClick={() => setShowReviewModal(false)} style={{ flex: 1, height: '46px', justifyContent: 'center' }}>
                تعديل يدوي
              </button>

              <button type="button" onClick={() => { setShowReviewModal(false); setAttachedFile(null); }} style={{ background: '#fee2e2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: '8px', padding: '0 1rem', cursor: 'pointer', fontWeight: 700, fontSize: '0.85rem' }}>
                رفض
              </button>
            </div>
          </div>
